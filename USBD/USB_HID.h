...
//lint -esym(621, USB_HID_COLLECTION_USB_HID_USAGESWITCH, USB_HID_COLLECTION_USB_HID_USAGEMODIFIER)  N:999
...
*     To be able to receive data from the host either an endpoint must
*     be allocated (EPOut) or a buffer must be provided (BufferSize, pBuff).
*     If EPOut == 0 and BufferSize == 0, then USBD_HID_Read() will not
...
*
*   Additional information
*     In case no EP Out was used with the HID interface, USBD_HID_Read
*     can be used to read the report that has been sent from the host.
*/
...
//lint -esym(621, USB_HID_IsStartReadTransferActiveEx)  N:999
